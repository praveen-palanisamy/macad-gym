...
class Net(nn.Module):
    def __init__(self, state_space, action_space):
        ...
        set_init([self.conv1, self.conv2, self.conv3, self.conv4, self.linear, self.critic_linear, self.actor_mu, self.actor_sigma])
    ...
    def forward(self, inputs):
        ...
        if len(inputs.shape) < 2: # Single input; batch_size == 1
            obs = obs.unsqueeze(0)
        input_image = obs[:,:self.input_image_size].contiguous().view(-1, self.input_image_shape[2], self.input_image_shape[0], self.input_image_shape[1])
...
class Worker(mp.Process):
    def __init__(self, gnet, opt, global_ep, global_ep_r, res_queue, name, env_creator, config, N_S, N_A):
        ...
        self.summary_writer = SummaryWriter(os.path.expanduser("~/tensorboard_log/continuous_a3c/{}_{}_{}_{}_{}_{}_{}".
format(now.year,now.month,now.day,now.hour,now.minute,now.second,self.name)))

    def run(self):
        while self.g_ep.value < self.config["MAX_EP"]:
            s = self.env.reset()
            s = torch.from_numpy(np.concatenate((s[0].flatten(), np.array([s[1]]), np.array(s[2]).flatten())))
            ...
            for t in range(self.config["MAX_EP_STEP"]):
                ...
                s_, r, done, py_measurements = self.env.step(a.squeeze().clip(-1, 1))
                times_sum += time.time()-start_time
                #TensorboardX freezing here
                #self.summary_writer.add_scalar("Current Reward",torch.DoubleTensor([r]),self.total_step)
                #self.summary_writer.add_scalar("Distance to Goal",torch.DoubleTensor([py_measurements["distance_to_goal_euclidean"]]),self.total_step)
                s_= torch.from_numpy(np.concatenate((s_[0].flatten(), np.array([s_[1]]), np.array(s_[2]).flatten())))
                ...
                if self.total_step % self.config["UPDATE_GLOBAL_ITER"] == 0 or done:  # update global and assign to local net
                    # sync
                    push_and_pull(self.opt, self.lnet, self.gnet, done, s_, buffer_s, buffer_a, buffer_r, self.config["gamma"])
                    buffer_s, buffer_a, buffer_r = [], [], []
            ...
            self.summary_writer.add_scalar("Number of Successfully Completed Episodes",torch.DoubleTensor([self.successful_episodes/self.g_ep.value]),self.g_ep.value)
            self.summary_writer.add_scalar("Mean Reward",torch.DoubleTensor([ep_r/(t+1)]),self.g_ep.value)
            self.summary_writer.add_scalar("Mean Time Per Iteration in Seconds",torch.DoubleTensor([times_sum/(t+1)]),self.g_ep.value)

            self.mean_episode_len += (episode_len - self.mean_episode_len) / self.g_ep.value
            self.summary_writer.add_scalar("Mean Episode Length",torch.DoubleTensor([self.mean_episode_len]),self.total_step)
...
class ContinuousA3CTune(Trainable):
    def _setup(self):
        ...
        if "load_checkpoint_path" in self.config and self.config["load_checkpoint_path"] is not None:
            self.restore(self.config["load_checkpoint_path"])
        self.gnet.share_memory()         # share the global parameters in multiprocessing
        self.opt = SharedAdam(self.gnet.parameters(), lr=0.0002)  # global optimizer
        self.global_ep, self.global_ep_r, self.res_queue = mp.Value('i', 0), mp.Value('d', 0.), mp.Queue()
        self.training_iteration = 0
        # parallel training
        self.workers = [Worker(self.gnet, self.opt, self.global_ep, self.global_ep_r, self.res_queue, i, self.env_creator, self.config, self.N_S, self.N_A) for i in range(self.config["num_local_workers"])]#mp.cpu_count())]
    ...
    def _save(self, checkpoint_dir):
        ...
        torch.save(self.gnet.state_dict(), "{}global/{}_{}.pth".format(checkpoint_dir,self.training_iteration,current_time))
