...
class TrafficLight(object):
 ...
 # https://github.com/carla-simulator/driving-benchmarks/blob/master/version084/benchmark_tools/benchmark_runner.py
    ...
    def _test_for_traffic_light(self, vehicle):
        """ 
        test if car passed into a traffic light, returning 'red' if it crossed a red light 
        ...
        for light in self.lights:
            if light is not None: 
                if not self._map.is_point_on_intersection([player_x, player_y, 38]):
                    ...
                    t1_vector, t1_dist = get_vec_dist(light_x, light_y, player_x, player_y)
                    if self._is_traffic_light_active(light, vehicle.transform.orientation) :
                        if is_on_burning_point(self._map, vehicle.transform.location) and t1_dist < 6.0 :
