...
class CarlaMap(object):
    ...
    def get_data_at(self, WorldLocation):
        ...
        indexx = clampFloatToInt(self.PixelPerCentimeter * pixle.x, 0,
                                 MAP_WIDTH - 1)
        indexy = clampFloatToInt(self.PixelPerCentimeter * pixle.y, 0,
                                 MAP_HEIGHT - 1)
        ...
        for vehicle in self._actors:
            ...
            while collision_flag and iteration < self.MAX_ITERATION:
                ...
                if p1 >= min1 and p1 <= max1:
                    ...
                    print(
                        'collision happens in x direction,  adding spwan x-location by %4.2f'
                        % self._vel_update(vel.x))
                    continue
                elif p2 >= min2 and p2 <= max2:
                    self._location.y += self._vel_update(vel.y)
                    print(
                        "collision happens in y direction, adding spwan y-location by %4.2f"
                        % self._vel_update(vel.y))
                    continue
                elif p3 >= min3 and p3 <= max3:
                    self._location.z += self._vel_update(vel.z)
                    print(
                        "collision happens in z direction, adding spwan z-location by %4.2f"
            ...
            print('  will spawn a vehicle at location: (%4.2f, %4.2f, %4.2f)' %
